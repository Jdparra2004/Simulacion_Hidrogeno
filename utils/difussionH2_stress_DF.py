...
# Initialize arrays function now accepts dt as an argument
def initialize_arrays(n, nt, C0, Ri, Ro, dt):
    Cold = np.zeros(n)
    C = np.zeros(n)
    Disp = np.zeros(n)
    sigma_r = np.zeros(n)
...
# Now we pass dt to initialize_arrays
Cold, C, Disp, sigma_r, sigma_t, epsi_r, epsi_t, A, ADisp, rhs, rhsDisp, r, t, H, Cflux, Hflux, HDisp, HStress_r, HStress_t, HStrain_r, HStrain_t = initialize_arrays(n, nt, C0, Ri, Ro, dt)
...
# Post processing de stress and strain
    ...
    for i in range(n):
        ...
        if i == n - 1:
            epsi_r[i] = (3 * Disp[i] - 4 * Disp[i - 1] + Disp[i - 2]) / (2 * dr)
        sigma_r[i] = (E / ((1. + nu) * (2. * nu - 1.))) * ((nu - 1.) * epsi_r[i] - nu * epsi_t[i] + (1. / 3.) * Omega * C[i])
        sigma_t[i] = (E / ((1. + nu) * (2. * nu - 1.))) * ((nu - 1.) * epsi_t[i] - nu * epsi_r[i] + (1. / 3.) * Omega * C[i])
        HStress_r[i, j] = sigma_r[i]
        HStress_t[i, j] = sigma_t[i]
        HStrain_r[i, j] = epsi_r[i]
        HStrain_t[i, j] = epsi_t[i]
