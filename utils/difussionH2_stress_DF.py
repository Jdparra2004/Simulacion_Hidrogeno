...
def solve_diffusion(A, rhs, Cold, dt, nt, Cin, Cout, dr, D, r, S, H, Hflux, n):
    ...
    for j in range(nt):
        ...
        for i in range(1, n-1):  # post processing de flujos
            Cflux[i, j] = -2.0 * np.pi * r[i] * D * (C[i + 1] - C[i - 1]) / (2 * dr)
        Cflux[0, j] = -2.0 * np.pi * r[0] * D * (-3 * C[0] + 4 * C[1] - C[2]) / (2 * dr)
        Cflux[n-1, j] = -2.0 * np.pi * r[n-1] * D * (3 * C[n-1] - 4 * C[n-2] + C[n-3]) / (2 * dr)
        H[:, j + 1] = C
        Hflux[:, j + 1] = Cflux[:, j]  # Asegúrate de que esto sea un vector de un solo tiempo
        Cold[:] = C
